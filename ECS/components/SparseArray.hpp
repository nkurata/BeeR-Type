...
template <typename Component>
    class sparse_array {
        public:
            ...
            size_type get_index(const Component& value) const {
                auto it = std::find_if(_data.begin(), _data.end(), [&](const value_type& opt) {
                    return opt.has_value() && opt.value() == value;
                });
                if (it != _data.end()) {
                    return std::distance(_data.begin(), it);
                }
                return static_cast<size_type>(-1); // Avoid signed/unsigned mismatch
            }
